...
int Modo = 0; //Declaracion variable "x" que se usará en la sentencia "switch" 
...
void loop() {
  ...
  if ((Nivel <= Maximo)&&(Nivel >= Minimo)){  //Si el nivel es mayor que el Minimo establecido mantiene la Bomba apagada mientras el nivel sea menor que el Maximo
    Modo = 0; //Mantiene el estado inactivo, con bomba apagada
  }else if ((Nivel < Minimo)&&(Nivel >= 0)){ //Si nivel esta por debajo del minimo
          Modo = 1; //pasa modo activo, llenado de tanque, con bomba enecendida
          }else{
              Modo = 2; //Fuera de rango esperado, estado de error, apaga bomba
          }
      
  switch(Modo){
    case 0: //Si el nivel es mayor que el Minimo establecido mantiene la Bomba apagada mientras el nivel sea menor que el Maximo
      lcd.clear();
      lcd.println("Nivel Aceptable ");
      do {
      Distancia = ping(TriggerPin, EchoPin); //Se llama a la funcion "ping" definida al final y guardo el valor que devuelve dicha funcion en variable Distancia
      Nivel = nivel(Distancia);  //Se llama a la funcion "nivel" definida al final y guardo el valor que devuelva esta funcion en variable "Nivel"
      apagarBomba(BombaPin);
    ...
    case 1: // Si el nivel esta por debajo del minimo establecido enciente la bomba hasta que alcamce el maximo (Tanque lleno)
      lcd.clear();
    ...
    case 2: //Valor fuera de rango esperado, situacion de error, apaga la bomba
      lcd.clear();
